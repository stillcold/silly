...
static int getfieldfromcalender(lua_State *L, const char *key, int defaultV){
  int isnum;
  ...
  if (!isnum) {  /* field is not an integer? */
    ...
    else if (defaultV < 0)  /* absent field; no default? */
      return luaL_error(L, "field '%s' missing in date table", key);
    res = defaultV;
  }
  else {
    if (!(-10000 <= res && res <= 10000))
...
static void setcalenderfield (lua_State *L, const char *key, int value) {
  lua_pushinteger(L, value);
  lua_setfield(L, -2, key);
}

static void setcalenderboolfield (lua_State *L, const char *key, int value) {
...
static void setcalenderallfields (lua_State *L, int year, int mon, int day, int isLeap) {
...
static int calender_solartolunar(lua_State *L){
  ...
  int solarYear = getfieldfromcalender(L, "year", 0);
  int solarMonth = getfieldfromcalender(L, "month", 0);
  int solarDay = getfieldfromcalender(L, "day", 0);
  ...
  setcalenderallfields(L, lunarYear, lunarMonth, lunarDay, isleap);
